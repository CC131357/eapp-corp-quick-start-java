...
public class IndexController {
    ...
    public ServiceResult login(@RequestParam(value = "authCode") String requestAuthCode) {
        ...
        Salary salary = iSalaryService.selectSalaryById(userId);
    ...
    @ResponseBody
    public ServiceResult updateStatus (@RequestParam(value = "authCode") String requestAuthCode,@RequestParam(value = "flag") String flag,@RequestParam(value = "content")String content) {
        //获取accessToken,注意正是代码要有异常流处理
        String accessToken = AccessTokenUtil.getToken();
        ...
        // 获得到userId之后应用应该处理应用自身的登录会话管理（session）,避免后续的业务交互（前端到应用服务端）每次都要重新获取用户身份，提升用户体验
        String userId = response.getUserid();
        String signStatus = null != flag && "Y".equals(flag) ? "已签收":"错误";
        Integer count = iSalaryService.updateStatusSalaryById(signStatus,content,userId);
        ...
        if (count>0){
            if(null != flag && "Y".equals(flag)){
                //工资正常
                serviceResult = ServiceResult.success("提交成功");
            }else{
                //工资异常
                serviceResult = ServiceResult.success("异常已提交系统，等待HR处理");
            }
        }else {
            serviceResult = ServiceResult.failure("E","操作失败");
        }
        return serviceResult;
